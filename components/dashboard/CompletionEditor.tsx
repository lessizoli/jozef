...
export default function CompletionEditor({ project, saving, onRun }: Props) {
  ...
  return <div className="mt-6 space-y-5">
    ...
    <section className="space-y-4 rounded-xl border border-slate-700 bg-slate-950/60 p-4">
      <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold text-slate-500">Átadás dátuma<input disabled={completed} type="date" value={data.handoverDate} onChange={(event) => setData({ ...data, handoverDate: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100" /></label><label className="text-xs font-semibold text-slate-500">Átadás felelőse<input disabled={completed} value={data.responsibleName} onChange={(event) => setData({ ...data, responsibleName: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100" /></label></div>
      <label className="block text-xs font-semibold text-slate-500">Átadási jegyzet<textarea disabled={completed} rows={3} value={data.handoverNotes} onChange={(event) => setData({ ...data, handoverNotes: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100" /></label>
      <label className="block text-xs font-semibold text-slate-500">Hibák és utómunkák<textarea disabled={completed} rows={3} value={data.defectNotes} onChange={(event) => setData({ ...data, defectNotes: event.target.value })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100" /></label>
      <label className="flex items-center gap-3 text-sm text-slate-300"><input disabled={completed} type="checkbox" checked={data.customerConfirmed} onChange={(event) => setData({ ...data, customerConfirmed: event.target.checked })} /> Az ügyfél az átadást visszaigazolta</label>
      {!completed && <div className="grid grid-cols-2 gap-2"><button disabled={saving} onClick={() => onRun(() => saveProjectCompletion(project.id, data), 'Az átadási adatok mentve.')} className="rounded-lg bg-slate-700 px-3 py-2 text-sm font-semibold disabled:opacity-40">Mentés</button><button disabled={saving} onClick={() => onRun(() => completeProjectHandover(project.id, data, project.modules.finance.enabled), 'Az átadás befejeződött.')} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold disabled:opacity-40">Átadás befejezése</button></div>}
    </section>
  </div>;
}
