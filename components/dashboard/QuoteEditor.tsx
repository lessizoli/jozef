  ...
  return (
    <div className="mt-6 space-y-5">
        ...
        {draft.items.length === 0 ? (
          ...
          <div key={item.id} className="rounded-xl border border-slate-300 bg-slate-100 p-4 text-slate-900">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-700">{index + 1}. tétel</span>
              <button type="button" onClick={() => removeItem(item.id)} className="text-xs font-semibold text-rose-700 hover:text-rose-900">Törlés</button>
            </div>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Típus</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Megnevezés</label>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Mennyiség</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Egység</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">Nettó egységár</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">ÁFA</label>
      ...
      <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-slate-900">
        <div className="flex justify-between text-sm text-slate-800"><span>Nettó összesen</span><strong>{money.format(totals.netTotal)} Ft</strong></div>
        <div className="mt-2 flex justify-between text-sm text-slate-800"><span>ÁFA összesen</span><strong>{money.format(totals.vatTotal)} Ft</strong></div>
        <div className="mt-3 flex justify-between border-t border-emerald-300 pt-3 text-lg font-semibold text-emerald-900"><span>Bruttó végösszeg</span><strong>{money.format(totals.grossTotal)} Ft</strong></div>
      </div>

      {!clientEmail && <p className="rounded-lg border border-amber-400 bg-amber-50 p-3 text-xs font-medium text-amber-900">Kiküldéshez előbb add meg az ügyfél e-mail-címét a Projektadatok fülön.</p>}
      <div className="grid gap-2 sm:grid-cols-3">
        <button type="button" disabled={saving} onClick={onSave} className="rounded-lg border border-sky-600 px-3 py-2 text-sm font-semibold text-sky-700 hover:bg-sky-50 disabled:cursor-not-allowed disabled:border-slate-300 disabled:text-slate-500">Mentés</button>
        <button type="button" disabled={saving} onClick={onDownload} className="rounded-lg border border-emerald-600 px-3 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:border-slate-300 disabled:text-slate-500">PDF letöltése</button>
        <button type="button" disabled={saving || !clientEmail} onClick={onSend} className="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 disabled:opacity-100">Mentés és kiküldés</button>
      </div>
    </div>
  );
}
