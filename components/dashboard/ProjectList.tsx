...
function ProjectRow({ project, stage, onOpenModule, onEditProject, onCloseProject }: { project: Project; stage: ModuleKey; onOpenModule: Props['onOpenModule']; onEditProject: Props['onEditProject']; onCloseProject: Props['onCloseProject'] }) {
  ...
  return <div className="border-t border-slate-200 transition hover:bg-slate-50/70">
    <div className="grid gap-4 px-5 py-4 lg:grid-cols-[minmax(220px,1.4fr)_minmax(150px,.8fr)_minmax(140px,.7fr)_auto] lg:items-center">
      <div className="min-w-0"><span className="rounded-md bg-sky-50 px-2 py-1 text-[10px] font-bold text-sky-700">{project.code}</span><h3 className="mt-2 truncate text-sm font-bold text-slate-800">{project.title}</h3><p className="mt-1 truncate text-xs text-slate-500">{project.client.name} · {project.client.address || t('Nincs cím megadva')}</p></div>
      <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('Aktuális szakasz')}</p><p className={`mt-1.5 text-xs font-bold ${isProjectDelayed(project) ? 'text-rose-600' : completedStatuses.includes(projectModule.status) ? 'text-emerald-600' : 'text-amber-600'}`}>{t(moduleLabels[stage])} · {t(stageStatus)}</p>{projectModule.scheduledAt && <p className="mt-1 text-xs text-slate-500">{projectModule.scheduledAt}{projectModule.scheduledTime ? ` · ${projectModule.scheduledTime}` : ''}</p>}<button type="button" onClick={() => onOpenModule(project, stage)} className="mt-2 rounded-md bg-sky-50 px-2.5 py-1.5 text-[11px] font-bold text-sky-700 hover:bg-sky-100">{t('{module} megnyitása', { module: t(moduleLabels[stage]) })}</button></div>
      <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('Felelős')}</p><p className="mt-1.5 text-xs font-semibold text-slate-600">{projectModule.assignedTo || t('Nincs hozzárendelve')}</p><p className="mt-1 truncate text-[11px] text-slate-400">{financeOverdue ? t('A számla fizetési határideje lejárt') : t(project.lastAction || 'Nincs korábbi művelet')}</p></div>
      <div className="flex flex-wrap gap-2 lg:justify-end">
        <Link href={`/projektek/${encodeURIComponent(project.id)}`} className="rounded-lg bg-sky-600 px-3 py-2 text-xs font-bold text-white shadow-sm hover:bg-sky-500">{t('Teljes projekt')}</Link>
        <button type="button" onClick={() => setQuickMenuOpen((open) => !open)} aria-expanded={quickMenuOpen} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-sky-700 hover:border-sky-300">{t('Gyorsmenü')} {quickMenuOpen ? '▴' : '▾'}</button>
      </div>
    </div>
    {quickMenuOpen && <div className="border-t border-sky-100 bg-sky-50/70 px-5 py-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2 border-t border-sky-100 pt-3">
        ...
        <Link href={`/dokumentumok?project=${encodeURIComponent(project.id)}`} className="rounded-lg bg-white px-3 py-2 text-xs font-semibold text-sky-700 shadow-sm hover:bg-sky-100">{t('Projektanyagok kezelése')}</Link>
        {!project.closed && <button type="button" onClick={() => onCloseProject(project)} className="rounded-lg bg-white px-3 py-2 text-xs font-semibold text-rose-600 shadow-sm hover:bg-rose-50">{t('Projekt lezárása')}</button>}
      </div>
    </div>}
  </div>;
}
