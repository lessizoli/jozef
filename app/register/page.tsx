...
export default function RegisterPage() {
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-950 px-4 py-10 text-slate-100">
    <section className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-7 shadow-2xl">
      ...
      <h1 className="mt-2 text-2xl font-bold">{t('Új cég létrehozása')}</h1>
      <p className="mt-2 text-sm text-slate-400">{t('Az első felhasználó automatikusan a cég adminisztrátora lesz. SuperAdmin jóváhagyása nem szükséges.')}</p>
      <form onSubmit={submit} className="mt-7 space-y-4">
        <label className="block text-sm text-slate-300">{t('Cég neve')}<input required value={companyName} onChange={(event) => setCompanyName(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-sky-500" /></label>
        <label className="block text-sm text-slate-300">{t('Adminisztrátor neve')}<input required value={fullName} onChange={(event) => setFullName(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-sky-500" /></label>
        <label className="block text-sm text-slate-300">{t('E-mail-cím')}<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-sky-500" /></label>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block text-sm text-slate-300">{t('Jelszó')}<input required minLength={8} type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-sky-500" /></label>
          <label className="block text-sm text-slate-300">{t('Jelszó ismét')}<input required minLength={8} type="password" autoComplete="new-password" value={passwordAgain} onChange={(event) => setPasswordAgain(event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-sky-500" /></label>
        </div>
        {error && <div role="alert" className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-3 text-sm text-rose-300">{error}</div>}
        <button disabled={submitting} className="w-full rounded-lg bg-sky-600 px-4 py-2.5 font-semibold transition hover:bg-sky-500 disabled:bg-slate-700">{submitting ? t('Cég létrehozása…') : t('Cég és admin létrehozása')}</button>
      </form>
      <p className="mt-5 text-center text-sm text-slate-400">{t('Már van hozzáférésed?')} <Link href="/login" className="font-semibold text-sky-400 hover:text-sky-300">{t('Belépés')}</Link></p>
    </section>
  </main>;
}
