  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><DashboardHeader view="project"/><div className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
    <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">{!project.closed && project.modules.survey.enabled && <Link href={`/?project=${encodeURIComponent(project.id)}&module=survey`} className="rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-sky-500">Felmérési űrlap</Link>}<Link href={`/dokumentumok?project=${encodeURIComponent(project.id)}`} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-sky-700 hover:border-sky-300">Anyagok kezelése</Link><a href="#folyamat" className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-sky-700 hover:border-sky-300">Projektfolyamat</a></div>
      </div>
    </header>
