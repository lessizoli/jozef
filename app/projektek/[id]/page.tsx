...
export default function FullProjectPage() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><DashboardHeader view="project"/><div className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
    ...
    {project.surveyData && <Section title="Felmérési eredmény" note="A helyszíni felméréskor rögzített adatok és képek."><div className="grid gap-5 md:grid-cols-2"><Info label="Ügyféligény" value={<p className="whitespace-pre-wrap">{project.surveyData.customerNeeds || 'Nincs megadva'}</p>}/><Info label="Helyszíni adottságok" value={<p className="whitespace-pre-wrap">{project.surveyData.siteConditions || 'Nincs megadva'}</p>}/><Info label="Méretek és mennyiségek" value={<p className="whitespace-pre-wrap">{project.surveyData.measurements || 'Nincs megadva'}</p>}/><Info label="Megjegyzés" value={<p className="whitespace-pre-wrap">{project.surveyData.notes || 'Nincs megadva'}</p>}/></div>{surveyImages.length > 0 && <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Felmérési képek</p><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{surveyImages.map((item) => <ProtectedThumbnail key={item.id} title={item.fileName || 'Felmérési kép'} load={() => getProtectedAttachmentPreview(item)} onOpen={() => void openProtectedAttachment(item)}/>)}</div></div>}</Section>}
