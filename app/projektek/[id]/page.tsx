...
import { getProjectModuleDisplayStatus, isProjectDelayed, subscribeToCompanyProjects, type Project } from '@/lib/projectService';
import { useI18n } from '@/lib/i18n';

const completedStatuses = ['Kész', 'Elfogadva', 'Aláírva', 'Befejezve', 'Fizetve'];

function currentModule(project: Project) {
...
function Info({ label, value }: { label: string; value?: React.ReactNode }) {
  const { t } = useI18n();
  return <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t(label)}</p><div className="mt-1.5 text-sm font-semibold text-slate-700">{typeof value === 'string' ? t(value) : value || t('Nincs rögzítve')}</div></div>;
}

function Section({ title, note, children }: { title: string; note?: string; children: React.ReactNode }) {
  const { t } = useI18n();
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-5"><h2 className="text-lg font-bold text-slate-900">{t(title)}</h2>{note && <p className="mt-1 text-sm text-slate-500">{t(note)}</p>}</div>{children}</section>;
}
...
function ProtectedThumbnail({ title, load, onOpen }: { title: string; load: () => Promise<string>; onOpen: () => void }) {
  ...
  return <button type="button" onClick={onOpen} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-slate-200 bg-slate-100 text-left shadow-sm hover:border-sky-400">
    {source ? <Image src={source} alt={title} fill unoptimized className="object-cover transition duration-300 group-hover:scale-105"/> : <div className="grid h-full place-items-center px-4 text-center text-xs text-slate-400">{failed ? t('Az előnézet nem tölthető be') : t('Kép betöltése…')}</div>}
...
export default function FullProjectPage() {
  ...
  const surveyImages = images.filter((item) => item.moduleKey === 'survey');
  ...
  if (!loaded) return <main className="grid min-h-screen place-items-center bg-slate-50 text-sm text-slate-500">{t('Projekt betöltése…')}</main>;
  if (!project) return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><div className="text-center"><h1 className="text-xl font-bold text-slate-900">{t('A projekt nem található')}</h1><Link href="/" className="mt-4 inline-block text-sm font-semibold text-sky-700">{t('← Vissza a projektekhez')}</Link></div></main>;

  const delayedProject = isProjectDelayed(project);
  ...
  const surveyTranslation = project.surveyData?.translations?.[language];
  const surveyValue = (key: 'customerNeeds' | 'siteConditions' | 'measurements' | 'notes') => surveyTranslation?.[key] || project.surveyData?.[key] || t('Nincs megadva');
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><DashboardHeader view="project"/><div className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
    <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-sky-50 px-2.5 py-1 text-xs font-bold text-sky-700">{project.code}</span><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${project.closed ? 'bg-slate-100 text-slate-600' : delayedProject ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{t(delayedProject ? 'Csúszásban' : project.status)}</span></div><h1 className="mt-3 text-2xl font-bold sm:text-3xl">{project.title}</h1><p className="mt-2 text-sm text-slate-500">{t(project.lastAction || 'Nincs korábbi művelet')}</p></div>
        <div className="flex flex-wrap gap-2">{!project.closed && <Link href={activeModuleUrl} className="rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-sky-500">{t('{module} megnyitása', { module: t(moduleLabels[activeModule]) })}</Link>}<Link href={`/dokumentumok?project=${encodeURIComponent(project.id)}`} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-sky-700 hover:border-sky-300">{t('Anyagok kezelése')}</Link><a href="#folyamat" className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-sky-700 hover:border-sky-300">{t('Projektfolyamat')}</a></div>
      </div>
    </header>

    <Section title="Alapadatok" note="Az ügyfél és a projekt legfontosabb adatai."><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4"><Info label="Ügyfél" value={project.client.name}/><Info label="Telefonszám" value={project.client.phone ? <a className="text-sky-700" href={`tel:${project.client.phone}`}>{project.client.phone}</a> : undefined}/><Info label="E-mail" value={project.client.email ? <a className="text-sky-700" href={`mailto:${project.client.email}`}>{project.client.email}</a> : undefined}/><Info label="Cím" value={project.client.address}/><Info label="Alapfeladat" value={project.initialTask}/><Info label="Létrehozva" value={readableDate(project.createdAt)}/><Info label="Utoljára frissítve" value={readableDate(project.updatedAt)}/><Info label="Csomag a létrehozáskor" value={project.moduleAccessSnapshot?.plan}/><Info label="Projektállapot" value={project.closed ? 'Lezárt' : delayedProject ? 'Csúszásban' : project.status}/></div></Section>

    {project.surveyData && <Section title="Felmérési eredmény" note="A helyszíni felméréskor rögzített adatok és képek."><div className="grid gap-5 md:grid-cols-2"><Info label="Ügyféligény" value={<p className="whitespace-pre-wrap">{surveyValue('customerNeeds')}</p>}/><Info label="Helyszíni adottságok" value={<p className="whitespace-pre-wrap">{surveyValue('siteConditions')}</p>}/><Info label="Méretek és mennyiségek" value={<p className="whitespace-pre-wrap">{surveyValue('measurements')}</p>}/><Info label="Megjegyzés" value={<p className="whitespace-pre-wrap">{surveyValue('notes')}</p>}/></div>{surveyImages.length > 0 && <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('Felmérési képek')}</p><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{surveyImages.map((item) => <ProtectedThumbnail key={item.id} title={item.fileName || t('Felmérési kép')} load={() => getProtectedAttachmentPreview(item)} onOpen={() => void openProtectedAttachment(item)}/>)}</div></div>}</Section>}

    <Section title="Projektfolyamat" note="Az összes munkaszakasz, időpont és felelős egyetlen áttekintésben."><div id="folyamat" className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{moduleKeys.map((key, index) => { const item = project.modules[key]; const status = getProjectModuleDisplayStatus(project, key); const complete = completedStatuses.includes(status); const delayed = ['Csúszás', 'Késedelem', 'Elutasítva'].includes(status); return <article key={key} className={`rounded-xl border p-4 ${!item.enabled ? 'border-slate-200 bg-slate-100 text-slate-400' : delayed ? 'border-rose-200 bg-rose-50' : complete ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}><p className="text-[10px] font-bold uppercase tracking-wider opacity-60">{t('{index}. szakasz', { index: index + 1 })}</p><div className="mt-2 flex items-center justify-between gap-3"><h3 className="font-bold">{t(moduleLabels[key])}</h3><span className="text-xs font-bold">{t(item.enabled ? status : 'Nem elérhető')}</span></div>{item.scheduledAt && <p className="mt-3 text-xs">{t('Időpont:')} {item.scheduledAt}{item.scheduledTime ? ` · ${item.scheduledTime}` : ''}</p>}{item.assignedTo && <p className="mt-1 text-xs">{t('Felelős')}: {item.assignedTo}</p>}{Boolean(item.completedAt) && <p className="mt-1 text-xs">{t('Elkészült:')} {readableDate(item.completedAt)}</p>}{!project.closed && item.enabled && <Link href={`/?project=${encodeURIComponent(project.id)}&module=${key}`} className="mt-4 inline-flex rounded-lg bg-white px-3 py-2 text-xs font-bold text-sky-700 shadow-sm ring-1 ring-slate-200 hover:ring-sky-300">{t('{module} megnyitása', { module: t(moduleLabels[key]) })}</Link>}</article>; })}</div></Section>
    ...
    <Section title="Képek, feljegyzések és dokumentumok" note={t('{images} kép · {notes} feljegyzés · {documents} dokumentum', { images: images.length + constructionPhotos.length, notes: notes.length + constructionLogs.length, documents: documents.length })}>
