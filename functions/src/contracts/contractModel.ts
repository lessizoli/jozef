...
export async function getContractContext(callerUid: string | undefined, projectId: unknown): Promise<ContractContext> {
  ...
  const configured = permissionSnap.data()?.roles?.[String(user.role)]?.manageContract;
  ...
  if ((configured ?? defaultAllowed) !== true) {
    throw new HttpsError('permission-denied', 'Nincs jogosultságod a Szerződés modul kezeléséhez.');
  }
